...
layout: post
title: "fabric2.0 实战1"
date: 2020-07-01 16:04:06 
description: "区块链"
tag: Blockchain
---

`本系列参考Mark_MMXIX https://blog.csdn.net/qq_28540443 教程，感谢博主的付出，本系列笔记中会对原博文中未尽之处做详细的介绍，欢迎大家提出问题。`

`本系列默认已经安装完成fabric2.0,所基于环境是ubuntu 18.04 go版本是1.14`

### 1.样例是最好的老师：byfn.sh

byfn.sh是官方提供的运行脚本

我们详细分析一下其过程可以帮助我们理解fabric的架构，我将以源码中添加注释的方式来进行说明。为了加以区别，我的注释会使用中文。我们重点看这个部分的代码：

```sh
function generateCerts() #生成配置文件和证书
function generateChannelArtifacts() #生成创始区块与通道文件
```
...
function generateCerts() {
  ...
  #和cryptogen工具对等的是CA服务，是一种动态的证书生产环境；在开发和测试阶段，
  #在环境中不需要部署CA，因此可以简单的使用cryptogen工具，当然在运行环境中也可以不使用CA服务器，
  #而继续使用cryptogen。（https://www.jianshu.com/p/9d031a0606b7）
  ############################
  if [ "$?" -ne 0 ]; then
  ...
  set -x
  cryptogen generate --config=./crypto-config.yaml #生成配置文件，crypto-config.yaml主要包含的fabric排序节点证书配置以及fabric组织证书配置。见（1）
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  ./ccp-generate.sh # 生成调用nodejs SDK的相关区块链配置文件。接（2）查看脚本具体内容
}
...
PeerOrgs: # 组织节点
  ...
  # Org1
  # ---------------------------------------------------------------------------
  - Name: Org1 #配置了两个组织，每个组织2套公私钥和证书，包含普通User（Admin不包含在计数中）数量为1
...
后面有一些关于通道文件配置和共识机制配置的内容不是很清楚，等用到的时候再在此处做详细解释。